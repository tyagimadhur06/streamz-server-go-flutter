...
func init() {
	os.Setenv("APP_ID", "79a8049ca73d4737b11fead59b776f87")
	os.Setenv("APP_CERTIFICATE", "dd2f3f34636f4fb682168729b57aae11")
...
func getRtmToken(c *gin.Context) {
	...
	// get param values
	channelName,uidStr, expireTimestamp, err := parseRtmParams(c)
	...
	rtmToken, tokenErr := rtmtokenbuilder.BuildToken(appID, appCertificate, uidStr, expireTimestamp ,channelName)
...
func getBothTokens(c *gin.Context) {
	...
	// generate rtmToken
	rtmToken, rtmTokenErr := rtmtokenbuilder.BuildToken(appID, appCertificate, uidStr, expireTimestamp,channelName)
...
func parseRtmParams(c *gin.Context) (channelName,uidStr string, expireTimestamp uint32, err error) {
	// get param values
	channelName = c.Param("channelName")
	uidStr = c.Param("uid")
	expireTime := c.DefaultQuery("expiry", "3600")
	...
	return channelName, uidStr, expireTimestamp, err
}
